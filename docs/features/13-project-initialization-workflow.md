...
Le Makefile centralise l'initialisation locale de SecureFiles et prepare les dependances necessaires avant le lancement du frontend ou du backend. Les cibles separees permettent aussi d'initialiser tout le projet avec une seule commande.
...
- `make init` demarre PostgreSQL, MinIO, RabbitMQ et ClamAV, compile le backend et installe les dependances frontend.
- `make front` installe les dependances frontend puis lance Vite.
- `make back` demarre les services Docker, compile le backend puis lance Spring Boot.
...
- Les cibles `init-infra`, `init-backend` et `init-frontend` isolent les preparations et evitent de dupliquer les commandes dans `front` et `back`.
...
- `make -n front`, `make -n back` et `make -n SPRING_PROFILES_ACTIVE=prod back` ont valide les sequences sans lancer de serveur longue duree.
